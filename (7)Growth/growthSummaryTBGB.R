...
this_out<-"outputSCA5"; runFolder<-"TBGB_JP";
...
plotPath<-paste(basePath,"\\..\\Figures\\testing\\Growth\\",this_out,sep="")

doingMedian<-TRUE

##also read in M from literature to compare
groupGrowthPars<-read.csv(paste(basePath,"..\\length2weights.csv", sep=""))
...
for(g in 1:ng){
  ...
  if(thisNumCohorts>=minCohorts & thisCode %in% groupGrowthPars$Code){
