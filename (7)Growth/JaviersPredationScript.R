...
##' 
##' 
source(paste(DIR$'General functions',"text2num.R",sep=""))

#plot all tracers for a given box and layer
this_run<-"base"
this_out<-"BH_hybridKLP4"

this_path<-paste(DIR$'Base',"ATLANTISmodels\\",this_run,"\\",sep="")
outPath<-paste(this_path,"output",this_out,"\\",sep="")

prm.file<-paste(this_path,"..\\CRAM_BH_hybrid_biol.prm",sep="")
grp.file<-paste(this_path,"..\\CRAM_groups.csv",sep="")
nc.file<-paste(outPath,"output.nc",sep="")
bgm.file<-paste(this_path,"..\\CHAT30_aea.bgm",sep="")
cum.depths<-c(200,400,600,800,1300,1500); thisNumLayers<-length(cum.depths)
...
feeding.mat.shy <- function(prm.file, grp.file, nc.file, bgm.file, cum.depths, quiet = TRUE){
  ...
  # numlayers  <- thisNumLayers
  numlayers$numlay<-numlayers$numlay+1
  ...
  shinyApp(
    ...
    function(input, output, session) {
      ...
      rff <- reactive({
        cat("real.feed dim ",dim(real.feed)," N.mat length ",length(N.mat$Ava),"---")
        t(log(real.feed * N.mat$Ava))
      })
      rff2 <- reactive({
        cat("real.feed dim ",dim(real.feed)," N.mat dim2 ",dim(N.mat),"---")
...
Bio.func <- function(nc.file, groups.csv, numlayers){
  ...
  Is.off <- which(groups.csv$IsTurnedOn == 0)
  FG     <- str_trim(as.character(groups.csv$Name),side="both")
  ...
  for(code in 1 : length(FG)){
    cat(as.character(groups.csv$Code[code]),"--")
      ...
      for(coh in 1 : groups.csv$NumCohorts[code]){
        ...
        if(all(is.na(N.tot)) || all(N.tot == 0) || sum(N.tot, na.rm = TRUE) == 0){
          ...
          if(length(dim(N.tot)) > 3){
            ...
            w.m2[is.infinite(w.m2)] <- NA
            Temp.N  <- N.tot* w.m2
            if(length(dim(Temp.N))==2){
              Temp.N<-Temp.N[,1]
            }
            # Temp.N   <- sum(Temp.N * w.m2, na.rm = TRUE)
