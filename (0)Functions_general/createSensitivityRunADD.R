createSensitivityRunADD<-function(thisRun,runLength="",thisPath,editVar,version="",editGroups, runPrmFile="CRAM_base_run_short",changes,proportionalChange,nextLine,isVector=FALSE,changeVectorIndex=NULL,groupsFileName,biolFileName,varMax,ICfile="CRAM_input.nc", forceFile="CRAM_force"){
  
  source(paste(DIR$'General functions',"get_first_number.R",sep=""))
  source(paste(DIR$'General functions',"get_biol_vector.R",sep=""))
  
  ## same as base version, but adds the changes to the existing values
  ...
  for(p in 1:(length(changes))){
    ...
    for(g in 1:(length(editGroups))){
      ...
      if(proportionalChange){
        ...
        if(isVector){
          thisNewValue<-thisBaseValue
          thisNewValue[changeVectorIndex]<-thisV + thisBaseValue[changeVectorIndex]
        }else{
          thisNewValue<-thisV
        }
      }
