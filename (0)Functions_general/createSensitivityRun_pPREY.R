createSensitivityRun_pPREY<-function(thisRun,runLength="",thisPath,editVar,version="",editGroups,changes,proportionalChange,nextLine,isVector=FALSE,changeVectorIndex=NULL,groupsFileName,biolFileName,varMax){
  ...
  all_pPREYVars<-thisBiolLines[grep("^pPREY",thisBiolLines)]
  ...
  for(p in 1:(length(changes))){
    ...
    for(g in 1:(length(editGroups))){
      ...
      #get all pPREY vars for this group (some have 1 some have 4)
      thisVars<-all_pPREYVars[grep(thisGroup,all_pPREYVars)]
      nvars<-length(thisVars)
      if(nvars>0){
        for(v in 1:nvars){
          thisVar<-thisVars[v]
          ...
          if(isVector){
            thisBaseVector<-get_first_number(thisLine,n="all")
            changeVI<-seq(1,length(thisBaseVector)) %in% changeVectorIndex
          }else{
            thisBaseValue<-get_first_number(thisLine)
          }
          if(proportionalChange){
            if(isVector){
              thisNewValue<-thisBaseVector
              thisNewValue[changeVI]<-min(thisNewValue[changeVI]*thisV,varMax)
            }else{
    ...
    # 
    runText<-paste("\nWD=\"$(pwd)\"\nRUN=\"../bin/bin/atlantisMerged -i CRAM_input.nc 0 -o output.nc -r CRAM_base_run.prm -f inputs/CRAM_force.prm -p inputs/CRAM_physics.prm -b CRAM_base_biol",editVar,version,p,".prm -s CRAM_Groups.csv -q inputs/CRAM_Fisheries.csv -d output",editVar,version,p,"\"\necho $RUN > RUN\nCMD=\"msub -l nodes=1 -l walltime=50:00:00 -l partition=slurm -l qos=standby -p -1000 -q large -o CRAM",editVar,p,".log.%j -e TBGB",editVar,version,p,".err.%j -S /bin/bash RUN\"\necho \"Running Atlantis ",editVar,version,p," for CRAM on MOAB in directory:\" $WD\necho -n \"Job started at: \" ; date\necho $RUN\nCOMMAND=\"cd $WD ; $CMD\"\nssh turbine $COMMAND\nsleep 0.5",sep="")
