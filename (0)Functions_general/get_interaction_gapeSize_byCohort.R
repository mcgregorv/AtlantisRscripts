get_interaction_gapeSize_byCohort<-function(predatorCode,preyCode,groupsDF,ThisIC.nc,plotPath,biolLines){
  ...
  #need output to be dim c(10,10)
  outGSbycohort<-array(NA,dim=c(10,10))
  ...
  if(pred_numCohorts>1 & prey_numCohorts>1){
    ...
    GSbycohort<-array(0,dim=c(length(preySN),length(lowerSNlimits)))
    ...
    for(i in 1:length(preySN)){
      for(j in 1:length(lowerSNlimits)){
        ...
        if(preySN[i]>=lowerSNlimits[j] & preySN[i]<=upperSNlimits[j]){ GSbycohort[i,j]<-1}
      }
    }
    outGSbycohort[1:length(lowerSNlimits), 1:length(preySN)]<-t(GSbycohort)
  }
  return(outGSbycohort)
  # return(limitProp)
}