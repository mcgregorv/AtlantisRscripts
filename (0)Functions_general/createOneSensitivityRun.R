createOneSensitivityRun<-function(thisRun,runLength="",thisPath,editVar,version="",editGroups, runPrmFile="CRAM_base_run_short",changes,proportionalChange,nextLine,isVector=FALSE,changeVectorIndex=NULL,groupsFileName,biolFileName,varMax,ICfile="CRAM_input.nc", forceFile="CRAM_force"){
  ...
  cat(paste("\n#",changes,sep=""),file=runFile,append=TRUE)
    ...
    for(g in 1:(length(editGroups))){
      ...
      thisNewValue <- thisNewValue[!is.na(thisNewValue)]
      if(nextLine){
