getPreyBiomassAvail<-function(thisPreyCode,thisPreyName,thisPreyNumCohorts,thisPreyAge,ThisIC.nc,volume,thisLayer,thisBox,biolLines,thisGapeL=NA,thisGapeU=NA){
  thisPreyBiomass<-0
  # cat("in second fn layer=",thisLayer,", box=",thisBox,", ")
  thisCellVol<-volume[thisLayer,thisBox]
  ##called in getAvailFood
  if(thisPreyNumCohorts>1){
    tempBiomassArray<-rep(0,thisPreyNumCohorts)
    for(c in 1:thisPreyNumCohorts){
      ...
      if(!is.na(thisGapeL) & !is.na(thisGapeU)){
        if(preySN>=thisGapeL & preySN<=thisGapeU){
          tempBiomassArray[c]<-preyNums[thisLayer,thisBox]*(preySN+preyRN)
        }
      }else{
        tempBiomassArray[c]<-preyNums[thisLayer,thisBox]*(preySN+preyRN)
      }
