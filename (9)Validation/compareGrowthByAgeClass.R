...
pdf(paste(plotPath,"SizeAtAgeALLgroups.pdf", sep=""))
       ...
       slength<-dim(thisLengths)[2]
        ...
        for(c in 1:thisNumCohorts){
          if(sum(test_l, na.rm=TRUE)>0){
            ...
            xx<-1.96*sqrt((thisAsd^2/117) + (thisBsd^2/117))
            thisCI<-(thisAmean - thisBmean) + c(-xx, xx)
            
            Z<-(thisAmean - thisBmean) / (sqrt((thisAsd^2/1) + (thisBsd^2/117)))
            thisPnorm<-pnorm(abs(Z))-0.5
            this_p<-2* (0.5-thisPnorm)
           
            #calc cv required to have zero within diff or means CI
            diffMeans<-thisAmean - thisBmean
            minAsd <- sqrt(((diffMeans/1.96)^2 - thisBsd/this_n) * 1)
            minCV<-minAsd/thisAmean

            storePbycohort[g,c]<-this_p
            storeCVrequiredByC[g,c]<-minCV
           }
        }
       thisYmax<-max(c(test_l, thisUpperCI),  na.rm=TRUE)
       # plot(x=seq(1,thisNumCohorts), y=rep(lengthsMax, thisNumCohorts), type="n", xlab="Age Class", ylab="Length (cm)", xaxt="n", ylim=c(0,thisYmax))
       # axis(at=seq(1,thisNumCohorts), labels=seq(1,thisNumCohorts), side=1)
       # for(c in 1:thisNumCohorts){
       #   points(x=rep(c,slength), y=thisLengths[c,], col=ptCol, pch=20)
       # }

       bpdata<-melt(thisLengths)
       boxplot(value ~ X1, data=bpdata, xlab="Age Class", ylab="Length (cm)", xaxt="n", ylim=c(0,thisYmax))
       axis(at=seq(1,thisNumCohorts), labels=seq(1,thisNumCohorts), side=1)

       points(x=seq(1,thisNumCohorts), y=test_l[1:thisNumCohorts], type="l", lwd=3, col=myOrange)
