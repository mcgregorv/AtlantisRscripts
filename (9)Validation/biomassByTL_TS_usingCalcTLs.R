...
groupsTL<-read.csv(paste(TLpath,"CRAMGroupsTL.csv",sep=""))
...
biomasTLdf<-biomasTLdf[biomasTLdf$Group != "DC",]
...
biomasTLdf$TLc[biomasTLdf$TLc > 5] <- 5
biomasTLdf$TLc[biomasTLdf$TLc < 1] <- 1
...
fitData<-data.frame(cbind("TL"=seq(1,length(biomassByTL)-1),"biomass"=logBiomassByTL[-1]))
...
ll_lm<-function(pars){
  a<-pars[1]; b<-pars[2]
  est<-unlist(lapply(seq(1,length(obs)), lm_fn, a=a, b=b))
  ss<-0
...
thisOptim<-optim(par=c(15,-1), fn=ll_lm)
...
pdf(paste(plotPath,"BiomassByTL.pdf",sep=""),height=4,width=5)
par(las=1,mar=c(4.5,5,1,1))
plot(as.double(names(logBiomassByTL))[-1],type="n",xlab="Trophic level",ylab="",ylim=c(0,thisMax),yaxt="n",xlim=c(0.5,5))
points(x=as.double(names(logBiomassByTL))[-1],y=logBiomassByTL[-1],type="h",lwd=5,lend=1,col=myOrange)
axis(at=yaxis_at,labels=yaxis,side=2)
...
pdf(paste(plotPath,"_BiomassByGroupAndTL.pdf",sep=""),height=4,width=14)
...
## fit to the decline
fitData<-data.frame(cbind("TL"=as.double(as.character(toPlot$TL)),"biomass"=log10(toPlot$BiomassTonnes)))

model<-lm(biomass ~ TL,data=fitData)
thisInt<-summary(model)$'coefficients'[1,1]
thisSlope<-summary(model)$'coefficients'[2,1]
fitLine<-unlist(lapply(fitData$TL,FUN=function(x){thisSlope*x+thisInt}))

plot(x=fitData$TL, y=fitData$biomass, pch=20)
points(x=fitData$TL, y=fitLine, type="l", col="red")

ll_lm<-function(pars){
  a<-pars[1]; b<-pars[2]
  est<-unlist(lapply(fitData$TL, lm_fn, a=a, b=b))
  ss<-0
  for(i in 1:dim(fitData)[1]){
    ss<-ss+ (fitData$biomass[i]-est[i])^2
  }
  return(ss)
}

# thisOptim<-optim(par=c(10,-5), fn=ll_lm, upper=c(0, Inf), lower=c(-Inf,6), method="L-BFGS-B")
...
fitData<-data.frame(cbind("TL"=seq(1,dim(toPlot)[1]),"biomass"=log10(toPlot$BiomassTonnes)))
thisOptim<-optim(par=c(10,-5), fn=ll_lm)
optim_a<-thisOptim$par[1]; optim_b<-thisOptim$par[2]
thisInt<-optim_b
thisSlope<-optim_a
...
thisText<-paste("log(Biomass)=",signif(thisInt, 2), "-",abs(round(thisSlope,2)),"x trophic level ranking", sep="")

pdf(paste(plotPath,"_BiomassByGroupAndTL_withLine.pdf",sep=""),height=4,width=14)
par(las=0,mar=c(5,5,1,1))
plot(x=seq(1,dim(toPlot)[1]),y=log10(toPlot$BiomassTonnes),type="h",lwd=3,col=myBlue,xlab="",ylab="Biomass (tonnes)",xaxt="n",yaxt="n",lend=1)
polygon(x=c(min(TL1index)-0.5,max(TL1index)+0.5,max(TL1index)+0.5,min(TL1index)-0.5),y=ypoly,col=myGreen_trans,border=NA)
polygon(x=c(min(TL2index)-0.5,max(TL2index)+0.5,max(TL2index)+0.5,min(TL2index)-0.5),y=ypoly,col=myBlue_trans,border=NA)
...
par(las=2)
axis(at=seq(1,dim(toPlot)[1]),labels=toPlot$Group,side=1,cex=0.15)
points(x=fitData$TL,y=fitLine, type="l", col="red", lwd=2)
text(thisText, x=52, y=9)
# mtext("TESTING", side=3)
dev.off()
