...
for(g in 1:ng){
  ...
  if(thisNumCohorts>1){
    ...
    if(thisCode %in% catchCodes){
      thisName<-str_trim(groupsDF$Name[groupsDF$Code==thisCode],side="both")
      thisEst<-storeSSBfish[g,]; thisEstBase<-storeSSBbase[g,]
      ...
      plot(x=plotBiomassYears,y=(thisEst[biomassIndex]),type="l",col="black",cex=thisCex,ylim=c(0,biomassMax),
           xaxt="n",xlab="",lwd=3,yaxt="n",ylab="", yaxt="n")
      points(x=plotBiomassYears, y=thisEstBase[biomassIndex], type="l",lty=1,lwd=1.5)
      thisBiomassAxis<-pretty(seq(0,biomassMax), length.out = 5)
...
## do legend seperately
thisPlotFile<-paste(plotPath,"EstObsAndTotalCatchbyYearLEGENDinclNoFishing",sep="")
pdf(paste(thisPlotFile,".pdf",sep=""),height=2, width=3)
# jpeg(paste(thisPlotFile, ".jpg", sep=""), quality=3000)
par(mar=c(0,0,0,0), lend=1)
makeBlankPlot()
legend(legend=c("Catches (forced)", "Model estimated biomass", "Model (no fishing)", "Trawl survey estimated biomass"), col=c(catchBarColor, "black", "black", "red"), lwd=c(4,3,1.5,1.5), 
       seg.len=2.5,  lty=1, pch=c(NA,NA, NA, 20), x="center", bty="n")
dev.off()


## create tex insert
#sort catchCodes by description
catchDescriptions<-groupsDFPaper$Name[groupsDF$Code %in% catchCodes]
catchCodesOdered<-catchCodes[match(sort(catchDescriptions), catchDescriptions)]
...
for(g in 1:ncg){
  thisCode<-catchCodesOdered[g]; thisFig<-paste("EstSSBObsAndTotalCatchbyYear", thisCode,".pdf", sep="")
