...
testTS<-150
timeIndex<-(modelStartYear-year0): ntsteps
...
plot(1, type="n", ylim=c(0,thisMax), xlim=c(0,7))
for(T in 1:6){
...
pdf(paste(DIR$'Figures',"Validation\\trophicLevels\\TS_BiomassByTL.pdf",sep=""),height=4,width=5)
par(las=1,mar=c(4.5,5,1,1))
...
#detritus CIs
y0=detritusLC; y1=detritusUC
points(x=c(0.5,0.5), y=c(y0,y1), type="l", lty=1, lwd=1.2)
segments(x0=0.5-0.05, y0=y0, x1=0.5+0.05, y1=y0, lwd=1.2); segments(x0=0.5-0.05, y0=y1, x1=0.5+0.05, y1=y1, lwd=1.2)
#bacteria CIs
y0=bacteriaLC; y1=bacteriaUC
points(x=c(0.7,0.7), y=c(y0,y1), type="l", lty=1, lwd=1.2)
segments(x0=0.7-0.05, y0=y0, x1=0.7+0.05, y1=y0, lwd=1.2); segments(x0=0.7-0.05, y0=y1, x1=0.7+0.05, y1=y1, lwd=1.2)
abline(v=0.90,col="red",lty=2,lwd=2)
par(las=2)
axis(at=c(0.5,0.7),labels=c("Detritus","Bacteria"),side=1)
dev.off()
